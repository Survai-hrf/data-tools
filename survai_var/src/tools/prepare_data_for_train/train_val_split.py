...
def train_val_split(split_path):
    ...
    def split_files(x):
        ...
        if x.split == 'val':
            #master_bucket.copy_blob(master_blob, master_bucket, f"datasets/var/{split_path}/val/{x.label}/{x.file}")
            val_txt.append(f"{x.label}/{x.file} {class_num}")

        if x.split == 'train':
            #master_bucket.copy_blob(master_blob, master_bucket, f"datasets/var/{split_path}/train/{x.label}/{x.file}")
